...
describe('Projects', () => {
  ...
  it('should render every project with its stack', () => {
    ...
    expect(items[1].querySelector('.projects__name')?.textContent?.trim()).toContain(
      'El Pollo Loco',
    );
  });
  ...
  it('should show the preview image only while a row is hovered', () => {
    ...
    expect(
      element.querySelector<HTMLImageElement>('.projects__preview-image')?.getAttribute('src'),
    ).toBe('images/pollo.png');
  });
  ...
  it('should open the overlay with number, name, question and image', () => {
    ...
    expect(
      element.querySelector<HTMLImageElement>('.projects__dialog-image')?.getAttribute('src'),
    ).toBe('images/join.png');
  });
